...
```bash
# 本地容器
docker build -t vvbot .
docker run -p 8000:8000 -e DASHSCOPE_API_KEY=sk-你的key vvbot
# → http://localhost:8000
```
...
这是 VV bot 的终极形态:把"网页里的 VV"塞进一只**会看会聊的毛绒玩具**。
